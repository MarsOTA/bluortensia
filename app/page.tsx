...
import { AtmosphereGrid, Benefits, CTA, FAQSection, InternalLinks, ParasolArt, PhilosophySection, PhotoCTA, ProductSection, faqs } from "@/components/Sections";
...
export default function Home() {
  ...
  return <main><JsonLd data={faqLd}/><JsonLd data={product}/>
    <section className="container-pad grid min-h-[82vh] items-center gap-14 py-16 lg:grid-cols-[0.95fr_1.05fr] lg:py-24">
      <div className="hero-copy"><p className="mb-5 text-sm font-semibold uppercase tracking-[0.32em] text-sage">Ombra artigianale per luoghi da ricordare</p><h1 className="heading text-5xl leading-[0.98] md:text-7xl">Ombrelloni artigianali per beach club, hotel e ville sul mare</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-ink/75">BluOrtensia crea teli crochet e macramé, ombrelloni completi e soluzioni su misura per dare al tuo outdoor una firma calda, mediterranea e immediatamente fotografabile.</p><div className="mt-9 flex flex-wrap gap-4"><CTA /><Link href="/contatti" className="btn btn-ghost">Parla con BluOrtensia</Link></div><p className="mt-6 max-w-xl text-sm leading-7 text-ink/55">Beach club, hotel, ville, giardini e ristoranti sul mare: ogni progetto parte da foto, misure e atmosfera del luogo.</p></div>
      <ParasolArt />
    </section>
    <PhilosophySection />
