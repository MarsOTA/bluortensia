...
export function Header() {
  return <header className="sticky top-0 z-50 border-b border-sand/30 bg-warm-white/85 backdrop-blur-xl">
    <div className="container-pad flex h-20 items-center justify-between gap-6">
      <Link href="/" className="flex items-center" aria-label="BluOrtensia home"><Image src={brandAssets.logo} alt={brandAltText.logo} width={260} height={45} priority className="h-auto w-44 sm:w-56" /></Link>
...
export function Footer() {
  return <footer className="border-t border-sand/30 bg-cotton py-12">
    <div className="container-pad grid gap-8 md:grid-cols-3">
      <div><Image src={brandAssets.logo} alt={brandAltText.logo} width={300} height={52} className="h-auto w-56" /><p className="mt-4 text-sm leading-7 text-ink/70">Ombrelloni crochet e macramé per outdoor mediterranei scenografici, riconoscibili e fotogenici.</p></div>
